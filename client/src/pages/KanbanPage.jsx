...
function JobCard({ job, isDragging, navigate }) {
  return (
    <div
      className={`bg-gray-800 border rounded-lg p-3 select-none transition-all ${
        isDragging
          ? 'border-brand-500 shadow-2xl shadow-brand-500/20 opacity-90 rotate-1 scale-105'
          : 'border-gray-700 hover:border-gray-600 cursor-grab active:cursor-grabbing'
      }`}
    >
      <div className="font-semibold text-white text-sm truncate">{job.company}</div>
      <div className="text-xs text-gray-400 mt-0.5 truncate">{job.job_title}</div>
      <div className="mt-2 flex items-center justify-between gap-2">
        <span className="text-xs text-gray-600">{job.application_date}</span>
        {job.next_follow_up && (
          <span className="text-xs text-orange-400 shrink-0">⏰ {job.next_follow_up}</span>
        )}
      </div>
      {job.salary_range && (
        <div className="text-xs text-gray-500 mt-1">{job.salary_range}</div>
      )}
      <button
...
export default function KanbanPage() {
  ...
  async function handleDragEnd({ active, over }) {
    ...
    if (overData?.type === 'column') {
      ...
      // Fallback: check if over.id is a column name
      if (COLUMNS.includes(over.id)) {
        targetStatus = over.id;
      } else {
        targetStatus = jobs.find(j => j.id === over.id)?.status;
      }
    }
    ...
    setJobs(prev => prev.map(j => j.id === activeId ? { ...j, status: targetStatus } : j));

    try {
      await jobsApi.update(activeId, { status: targetStatus });
      toast.success(`Moved to ${targetStatus}`, { duration: 1500 });
    } catch {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-500">{jobs.length} application{jobs.length !== 1 ? 's' : ''}</span>
      </div>
